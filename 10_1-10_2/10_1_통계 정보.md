...
- 그래서 이후 버전부터 실제 테이블의 데이터를 일부 분석해서 통계 정보를 보완해서 사용
...
- **STATS_PERSISTENT** 라는 옵션을 통해 영구적으로 보관 여부를 테이블을 생성할 때 세팅 가능
    ...
    - STATS_PERSISTENT=DEFAULT : 테이블을 생성할 떄 별도로 STATS_PERSISTENT 옵션을 설정하지 않은 것과 동일하며, 영구 관리 여부를 innodb_stats_persistent 시스템 볒ㄴ수의 값으로 결정
